...
- `OLLAMA_IMAGEGEN_ALLOW_OVERSUBSCRIBE` - When set to `1` (or `true`), allows image generation models to proceed even if required model memory is slightly above detected available memory. This can help borderline unified-memory systems load larger image models, but may cause slower inference, heavy swap pressure, or process/system instability.
